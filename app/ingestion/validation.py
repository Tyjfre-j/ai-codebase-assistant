...
def file_is_binary(content: bytes) -> bool:
...
def file_is_minified(file_path: str, content: bytes) -> bool:
    ...
    # Drop the last line if it's likely a partial line cut off by sampling.
    if len(lines) > 1:
        lines = lines[:-1]
