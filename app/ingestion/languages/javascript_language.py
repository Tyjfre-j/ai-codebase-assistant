...
def _unwrap_arrow_field(definition_node):
    """If this arrow function is the value of a class field, return the enclosing
    field_definition node instead — that's what's actually parented under class_body."""
    if (
...
def get_class_member_stub_info(node, content: bytes):
    """Return a class-member signature for JS class skeleton chunks.

    Handles both `method_definition` members and class-field arrow functions
    (`field_definition` whose value is an arrow_function). The latter are
    already chunked individually as their own DEFINITIONs via the QUERY's
    field_definition pattern, but were previously always excluded from the
    class's own skeleton listing, silently under-representing its member set.
    """
    ...
    if node.type == _FIELD_DEFINITION_TYPE:
        ...
        if value is None or value.type != "arrow_function":
            return None
        name = node_text(node.child_by_field_name("property"), content)
